  ...
  "relative w-full rounded-lg border p-4 text-sm [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg~*]:pl-7",
  {
    variants: {
      variant: {
        ...
        warning: "border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-800 dark:bg-amber-950/50 dark:text-amber-300 [&>svg]:text-amber-800 dark:[&>svg]:text-amber-300",
        success: "border-green-200 bg-green-50 text-green-800 dark:border-green-800 dark:bg-green-950/50 dark:text-green-300 [&>svg]:text-green-800 dark:[&>svg]:text-green-300",
      },
    },
    defaultVariants: {
      variant: "info",
    ...
    <div className="flex items-start gap-3">
      {showIcon && (
        <>
          ...
          {variant === "warning" && <AlertTriangle className="h-5 w-5 flex-shrink-0" />}
          {variant === "success" && <MdCheckCircle className="h-5 w-5 flex-shrink-0" />}
        </>
      )}
      {props.children}
    </div>
